...
def get_ripl_names(dat_directory):
    """Searches directory for RIPL dat files and returns a list of relative paths for each one.

    Args:
        dat_directory (str): path to the ENSDF directory containing all dat files.

    Returns:
        list: contains relative path to each dat file. None if no files are found.
    ...
    if len(names) != 0:
        ...
        logging.error("RIPL: No .dat files found. Make sure you specified the path correctly.")
        sys.exit()
        return None

def get_header(dat_list, saving_directory):
    ...
    """
    if len(dat_list) != 0:
        logging.info("HEADER: Extracting ENSDF headers ...")
        ...
        logging.info("HEADER: Finished. Saved to {}".format(header_file))
        return None
    else:
        logging.error("RAW HEADER: No .dat files found. Make sure you specified the path correctly.")
        sys.exit()
        return None
...
def generate_elemental_ensdf(dat_list, header_directory, saving_directory):
    ...
    Args:
        ...
        saving_directory (str): path where the new ENSDF files will be stored
    ...
    if os.path.exists(csv_file):
        ...
        general_utilities.initialize_directories(ensdf_v1_path)
        ...
        general_utilities.initialize_directories(ensdf_v2_path)
        ...
        general_utilities.initialize_directories(ensdf_v3_path)
        ...
        logging.error("ENSDF Elemental: The all_ensdf_headers_formatted.txt file was not found in the given path.")
        sys.exit()
        return None
...
def get_stable_states(dat_list, header_directory, saving_directory=None):
    ...
    Args:
        ...
        saving_directory (str): path where to save the resulting CSV file.
    ...
    if os.path.exists(csv_file):
        ...
        logging.error("STABLE STATES: No .dat files found. Make sure you specified the path correctly.")
        sys.exit()
        return None
...
def generate_ensdf_csv(header_directory, elemental_directory, saving_directory=None):
    ...
    Args:
        ...
        saving_directory (str): path where the new ENSDF .csv file will be stored
    ...
    if os.path.exists(csv_file):
        ...
        logging.info("Creatign DataFrame with Basic ENSDF data ...")
        appended_data = []
        ...
        logging.info("Finished creating list of dataframes.")
...
def get_level_parameters(level_params_directory, saving_directory=None):
    ...
    Args:
        ...
        saving_directory (str): path to directory where the resulting CSV will be stored.
    ...
    if os.path.exists(data_file):
        ...
        logging.info("RIPL: Parsing and formatting level parameters...")
            ...
            for line in infile:
                if line.strip():
                    ...
                    for i, j in enumerate([4, 8, 11, 21, 31, 41, 51, 55, 59, 63, 67, 76, 85, 96, 98, 100, 104, 116]):
                        string.insert(i + j, ';')
                    outfile.write("".join(string))
        logging.info("RIPL: Finished formating data. Converting to CSV...")
        ...
        logging.info("RIPL: Finished.")
    else:
        logging.error("RIPL: The levels-param.data file was not found. Make sure you specified the path correctly.")
        sys.exit()
        return None
...
def generate_cutoff_ensdf(ensdf_directory, elemental_directory, ripl_directory=None, saving_directory=None):
    ...
    the CSV file created using the get_level_parameters function from the RIPL parsing utilities. 

    Args:
        ensdf_directory (str): path to the directory where the ensdf.csv file is stored
        ripl_directory (str): path to the directory where the ripl_cut_off_energies.csv file is stored. Defaults to None.
        elemental_directory (str, optional): path to the Elemental_ENSDF_no_Header_F directory.
    ...
    if general_utilities.check_if_files_exist(files):
        ...
        logging.error("ENSDF CutOff: One or both files where not found. Make sure you specified both paths correctly.")
        sys.exit()
        return None