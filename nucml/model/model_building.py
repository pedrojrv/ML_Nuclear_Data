...
def train_knn(x_train, y_train, x_test, y_test, k_list, save_models=False, save_dir="./"):
    ...
    if save_models:
        model_error_metrics = pd.DataFrame(columns=["id", "train_mae", "train_mse", "train_evs", "train_mae_m", 
            "train_r2", "test_mae", "test_mse", "test_evs",  "test_mae_m", "test_r2", "model_path", "training_time"])
    else:
        model_error_metrics = pd.DataFrame(columns=["id", "train_mae", "train_mse", "train_evs", "train_mae_m", 
            "train_r2", "test_mae", "test_mse", "test_evs",  "test_mae_m", "test_r2", "training_time"])
...
def train_dt(x_train, y_train, x_test, y_test, parameters_dict, save_models=False, save_dir="./"):
    # Creating DataFrame to store performance regression metrics
    if save_models:
        model_error_metrics = pd.DataFrame(columns=["id", "max_depth", "mss", "msl","train_mae", "train_mse", 
