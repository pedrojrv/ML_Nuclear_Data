...
def generate_exfor_dataset(user_path, modes=["neutrons", "protons", "alphas", "deuterons", "gammas", "helions"]):
    ...
    helion-induce reactions. Beware, NucML configuration needs to be performed first. See nucml.configure.
    The `modes` argument can be modified for the function to generate only user-defined datasets.
...
def load_ame(natural=False, imputed_nan=False, file="merged"):    
    ...
    1. AME_all_merged (natural=False, imputed_nan=False): Contains all avaliable AME information from the mass, rct1, and rct2 files. 
    2. AME_all_merged_no_NaN (natural=False, imputed_nan=True): Same as 1, except all missing values are imputed linearly and element-wise.
    3. AME_Natural_Properties_w_NaN (natural=True, imputed_nan=False): Similar to 2, except data for natural abundance elements is included.
    4. AME_Natural_Properties_no_NaN (natural=True, imputed_nan=True): Same as 3. except all missing values are imputed linearly and element-wise.
...
####################### EVALUATED LIBRARIES ###################################
###############################################################################
def load_evaluation(isotope, MT, mode="neutrons", library="endfb8.0", mev_to_ev=True, mb_to_b=True, log=True, drop_u=True):
    """Reads an evaluation file for a specific isotope, reaction channel, and evaluated library. It is important
    to inspect the returned data since it queries a local database of an external source which extracted data from ENDF 
    using an extraction script. It has been found that some particular reactions are not included. These can be added 
    manually for future loading. 
    ...
    path = os.path.join(evaluations_path, '{}/{}/{}/tables/xs/{}-{}-{}.{}'.format(mode, isotope, library, projectile, isotope, MT, library))    
...
def load_ensdf(cutoff=False, append_ame=False):
    ...
    Args:
        ...
        append_ame (bool, optional) If True, AME isotopic properties will be appended to the loaded ENSDF data. Defaults to False.
